...
void myapp_config_load(myapp_config_t *cfg) {
    ...
    cfg->vlm_port       = 8002;
    cfg->homework_port  = 8003;
    cfg->translate_port = 8004;

    FILE *fp = fopen(CONFIG_PATH, "r");
    if (!fp) {
        printf("[config] %s not found, use defaults: "
               "wifi=%s iface=%s host=%s vlm=%d hw=%d tr=%d\n",
               CONFIG_PATH, cfg->wifi_ssid, cfg->wifi_iface,
               cfg->server_host, cfg->vlm_port,
               cfg->homework_port, cfg->translate_port);
        return;
    }
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        *eq = '\0';
        ...
        } else if (strcmp(key, "vlm_port") == 0) {
            cfg->vlm_port = atoi(val);
        } else if (strcmp(key, "homework_port") == 0) {
            cfg->homework_port = atoi(val);
        } else if (strcmp(key, "translate_port") == 0) {
            cfg->translate_port = atoi(val);
        }
    }
    fclose(fp);
    printf("[config] loaded: wifi=%s iface=%s host=%s vlm=%d hw=%d tr=%d\n",
