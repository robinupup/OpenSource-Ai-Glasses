...
static void handle_json(const char *text, size_t len) {
    ...
    if (strcmp(type, "asr") == 0) {
        ...
        int sr   = cJSON_IsNumber(jsr)   ? jsr->valueint   : 22050;
        ...
        /* realtime_translate 是连续翻译：每说完一句 server 都会发 done；
...
static void on_ws(ws_event_t ev, ws_msg_type_t type, const void *data,
    ...
    if (ev == WS_EV_OPEN) {
        ...
        /* 服务端常在 done 后主动关闭：标记需要重连，下次按键立即重开。*/
        g_cur_tts_seq = 0;
        tts_player_stop();
        pthread_mutex_lock(&g_lock);
        g_state = ST_IDLE;
...
static int ensure_ws_open(void) {
    ...
    char url[256];
    myapp_config_build_url(url, sizeof(url), g_host, g_port, "/realtime_translate");
...
void app_translate_on_confirm(void) {
    ...
    if (s == ST_IDLE || s == ST_RESULT || s == ST_WAIT) {
